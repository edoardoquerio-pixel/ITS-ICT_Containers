...
async def create_loan(loan_in: LoanCreate, db: Session = Depends(get_db)):
    """Crea un nuovo prestito con verifiche inter-servizio."""
...
async def return_loan(loan_id: int, db: Session = Depends(get_db)):
    """Registra la restituzione di un prestito. Aggiorna book-service."""
    return await loan_service.return_loan_async(db, loan_id)
